...
class MorningReversal(Strategy):
            ...
            if len(data) > 0:
                ...
                if len(prev_day_data) > 0:
                    ...
                    if len(prev_day_full) > 0:
                        ...
                        # Log the levels found
                        print(f"Previous day RTH levels - High: {levels.get('prev_day_high', 'N/A')}, Low: {levels.get('prev_day_low', 'N/A')}")
                        print(f"Previous day ETH levels - High: {levels.get('post_hours_high', 'N/A')}, Low: {levels.get('post_hours_low', 'N/A')}")
    ...
    def optimize(self, data: pd.DataFrame, objective_func: callable, 
        ...
        # Note: Using a more selective approach to avoid combinatorial explosion
        ...
        for i in range(1, len(data)):
            ...
            if not in_position:
                ...
                if signals[i] != 0:
                    ...
                    # Set stop loss
                    stop_loss = entry_price - (position_type * self.parameters['stop_atr_multiplier'] * data['atr'].iloc[i])
